...
static void *clearMemoryBlocks(void * b);
...
void destroyVulkanAllocators(Window *window) {
    for (int i=0; i<2*memProperties.memoryTypeCount; i++) {
        ...
        while (segments) {
            ...
            listIter(s->mem_base, clearMemoryBlocks);
            listDestroyList(s->mem_base);
        ...
        listDestroyList(allocators[i].segment_base);
        allocators[i].size = 0;
